...
#ifdef NO_AIO
    cobs::query::compact_index::mmap s(p);
#else
...
void add_command_rnd(CLI::App& app, std::shared_ptr<parameters> p) {
    ...
    sub->set_callback([p]() {
                          ...
                          for (size_t i = 0; i < p->num_warmup_exps; i++) {
                              warmup_queries.push_back(cobs::random_sequence(p->num_kmers + 30, (size_t)time(nullptr)));
                          }
                          for (size_t i = 0; i < p->num_exps; i++) {
                              queries.push_back(cobs::random_sequence(p->num_kmers + 30, (size_t)time(nullptr)));
                          }
