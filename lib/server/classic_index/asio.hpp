...
//                requests[i] = m_file.aread(rows, hashes[i] * m_h.block_size(), m_h.block_size());
//            }
//
...
//
//            auto* rows = reinterpret_cast<char*>(stxxl::aligned_alloc<4096>(m_h.block_size() * hashes.size()));
...
//                size_t j = 1;
//                while (j * 8 <= m_h.block_size()) {
//                    count_64[j - 1] &= rows_64[j - 1];
//                    j++;
//                }
//                j = (j - 1) * 8;
//                while (j < m_h.block_size()) {
//                    count[j] &= rows[j];
//                    j++;
//                }
//                if (i % m_h.num_hashes() == m_h.num_hashes() - 1) {
//                    for (size_t k = 0; k < m_h.block_size(); k++) {
...
//            std::ifstream ifs;
//            m_h = file::deserialize_header<file::classic_index_header>(ifs, path);
