...
namespace cobs::file {
    ...
    template<uint32_t N>
    void serialize(const fs::path& p, const sample<N>& s, const std::string& name);
    ...
    template<uint32_t N>
    void deserialize(const fs::path& p, sample<N>& s, sample_header& h = dummy_sh);
...
namespace cobs::file {
    ...
    template<uint32_t N>
    void deserialize(const fs::path& p, sample<N>& s, sample_header& h) {
